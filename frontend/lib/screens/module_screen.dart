          ...
          if (_loading)
            ...
            Card(
              child: Padding(
                ...
                child: Center(
                  child: Text(
                    'No ${widget.title.toLowerCase()} records yet. Click "${_addLabel()}" to create one.',
                  ),
                ),
              ),
            )
...
class _AddDialogState extends State<_AddDialog> {
  ...
  Map<String, dynamic>? _buildBody() {
    switch (widget.title) {
      case 'Project Tracking':
        if (_c('name').text.trim().isEmpty) return null;
        return {
      ...
      case 'Budget Management':
        if (_c('category').text.trim().isEmpty ||
            _c('project_id').text.trim().isEmpty) return null;
        return {
          'project_id': int.tryParse(_c('project_id').text.trim()),
      ...
      case 'Worker Attendance':
        if (_c('worker_name').text.trim().isEmpty ||
            _c('project_id').text.trim().isEmpty) return null;
        return {
          'project_id': int.tryParse(_c('project_id').text.trim()),
      ...
      case 'Progress Photos':
        if (_c('project_id').text.trim().isEmpty ||
            _c('photo_url').text.trim().isEmpty) return null;
        return {
          'project_id': int.tryParse(_c('project_id').text.trim()),
      ...
      case 'Client Reporting':
        if (_c('project_id').text.trim().isEmpty ||
            _c('title').text.trim().isEmpty) return null;
        return {
          'project_id': int.tryParse(_c('project_id').text.trim()),
    ...
    switch (widget.title) {
      case 'Project Tracking':
        return [
          ...
          _field('progress', 'Progress (%)', number: true),
          _field('start_date', 'Start date (YYYY-MM-DD)'),
          _field('end_date', 'End date (YYYY-MM-DD)'),
        ];
      case 'Budget Management':
        return [
          _field('project_id', 'Project ID', number: true),
          _field('category', 'Budget category'),
          _field('budget_amount', 'Budget amount (GH¢)', number: true),
          _field('actual_amount', 'Actual amount (GH¢)', number: true),
        ];
      case 'Worker Attendance':
        return [
          _field('project_id', 'Project ID', number: true),
          _field('worker_name', 'Worker name'),
          _field('attendance_date', 'Date (YYYY-MM-DD)'),
          DropdownButtonFormField<String>(
            ...
            onChanged: (v) =>
                setState(() => _attendanceStatus = v ?? 'Present'),
          ),
          const SizedBox(height: 12),
          _field('check_in', 'Check-in (HH:MM)'),
          _field('check_out', 'Check-out (HH:MM)'),
        ];
      case 'Material Management':
        return [
          _field('project_id', 'Project ID', number: true),
          _field('name', 'Material name'),
          _field('unit', 'Unit (bags, lengths, units...)'),
          _field('quantity_received', 'Quantity received', number: true),
          _field('quantity_used', 'Quantity used', number: true),
      ...
      case 'Progress Photos':
        return [
          ...
          _field('taken_at', 'Date/time (ISO format, optional)'),
        ];
      case 'Client Reporting':
        return [
          _field('project_id', 'Project ID', number: true),
          ...
          _field('report_date', 'Report date (YYYY-MM-DD)'),
        ];
      default:
        return [];
    }
